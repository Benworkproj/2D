...
<body>
	<div class="bg">	
		<a href="newpost.php"><button class="create"><i class="fa fa-plus" aria-hidden="true" style="color: darkgreen;"></i> | Add Post</button>
	<!-- PHP CODE BLOCK -->
		...
		<?php
			...
			echo "<center><table border = '2' width = '900px' style = 'border-collapse: collapse;'>
					<thead style = 'background-color: green;'>
						<th><center>Article ID</center></th>
			...
			foreach ($users as $users) {
				echo "<tr>";
    			echo "<td>{$users['art_id']}</td>";
    			echo "<td>{$users['art_name']}</td>";
    			echo "<td>{$users['art_cat']}</td>";
    			echo "<td>{$users['art_pubdate']}</td>";
   				echo "<td>{$users['art_author']}</td>";
   				echo "<td>{$users['art_desc']}</td>";
   				echo "<td>{$users['art_image']}</td>";
